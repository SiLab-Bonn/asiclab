# Install and enable essential repos
...
sudo dnf install -y fxload libusb1 libusb1-devel libusb #do we need other libusb compat?
...
# Remove the default outdated LibreOffice that AlmaLinux comes with; we instead want versions from Flatpak below
...
# Packages needed for Ramon:
sudo dnf install -y gtkwave

# Build requirements for iverilog
sudo dnf install -y autoconf gperf make gcc g++ bison flex 
# then need to git clone and install iverilog itself

# finally, maybe we should install ni-software-2023
# This script was run: /tools/xilinx/14.7/ISE_DS/common/bin/lin64/digilent/ftdi.drivers_1.0.4-x86_64/install.sh
